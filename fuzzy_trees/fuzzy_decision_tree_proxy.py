...
from fuzzy_trees.util_comm import get_timestamp_str
...
class FuzzyDecisionTreeProxy(DecisionTreeInterface):
    ...
    def __init__(self, fdt_class=None, disable_fuzzy=False, X_fuzzy_dms=None, fuzzification_params=None,
        ...
        self.enable_pkl_mdl = False  # Whether enable pickling fitted models?
    ...
    # =============================================================================
    # Pretraining and Plotting Functions
    # =============================================================================
    def pretrain_clf(self, ds_name_list, conv_k_lim, fuzzy_reg_lim):
        """
        ...
        self._encapsulate_save_data_clf(q=q)

    def _get_one_mean_clf(self, q, ds_name, conv_k, fuzzy_reg):
        """
        Fit a group of classifiers on a specified dataset and get the mean of
    ...
    def _fit_one_clf(self, X_train, X_test, y_train, y_test, ds_name, conv_k, fuzzy_reg):
        """
        Fit a classifier and get its evaluation scores.
        ...
        if self.enable_pkl_mdl:
            filename = DirSave.MODELS.value + "clf_" + ds_name + "_" + conv_k + "_" + fuzzy_reg + ".mdl"
    ...
    def _encapsulate_save_data_clf(self, q):
        """
        Encapsulate and save all data received from the child processes.
        ...
        if self.ds_pretrain is not None:
            ...
            filename = DirSave.EVAL_DATA.value + get_timestamp_str() + "_" + EvaluationType.FUZZY_REG_VS_ACC_ON_CONV_K.value + ".csv"
            self.df_pretrain.to_csv(filename)
        print("+++++++++++++++++++++++++++", np.shape(self.ds_pretrain), self.df_pretrain.shape)

    def plot_fuzzy_reg_vs_err_sep(self, filename=None):
        """
        Plot fuzzy regulation coefficient versus training error and
        test error based on different numbers of fuzzy clusters in
        separately figures.
        ...
        filename: str, default None
            If filename is not None, fetch the data from the file it
            specifies, otherwise try from memory and the latest file
    ...
    def plot_fuzzy_reg_vs_err(self, filename=None):
        ...
        """
        # Fetch the data for plotting from the specified file if filename is not None.
        if filename is not None:
            self.df_pretrain = pd.read_csv(filename)

        # Otherwise try from memory and the latest file in the default directory in turn.
        if self.df_pretrain is None:
            filename_list = os.listdir(DirSave.EVAL_DATA.value)  # NB: The list returned by listdir() is in arbitrary order.
        ...
        assert self.df_pretrain is not None, "No data for plotting. Please execute the function pretrain() first."
        # q.put([[ds_name, conv_k, fuzzy_reg, acc_train_mean, std_train, acc_test_mean, std_test]])
        ...
        for ds_name in ds_names:
            ...
            plot_multi_lines_subplots(df=df_4_ds_name,
                                      ...
                                      y_label="Error Rate",
                                      legends=["Train", "Test"],
                                      f_name=DirSave.EVAL_FIGURES.value + EvaluationType.FUZZY_REG_VS_ACC_ON_CONV_K.value + "_" + ds_name + ".png")

    def _fit_one_regr(self, X_train, X_test, y_train, y_test):
        """
        Fit a regressor and get its evaluation scores.
